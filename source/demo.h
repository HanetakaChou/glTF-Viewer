...
struct Demo_Mesh_Instance
{
	...
	std::vector<Demo_Mesh_Skinned_Subset> m_subsets;
	brx_descriptor_set *m_gbuffer_pipeline_per_mesh_instance_update_descriptor_set;
};

static constexpr uint32_t const DEMO_MESH_SUBSET_GEOMETRY_METERIAL_BUFFER_COUNT = 4U;
...
struct Demo_Mesh_Subset
{
	...
	brx_storage_asset_buffer *m_vertex_skinned_binding_buffer;
...
{
	bool m_skinned;
	std::vector<Demo_Mesh_Subset> m_subsets;
	std::vector<Demo_Mesh_Instance> m_instances;
};
...
class Demo
{
	...
	std::vector<Demo_Mesh> m_scene_meshes;
...
public:
	...
	void init(brx_device *device, std::vector<std::string> const &file_names);
