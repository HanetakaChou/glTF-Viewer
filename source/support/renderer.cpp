...
public:
	...
	void init(void *wsi_connection, char const *file_name);
...
extern renderer *renderer_init(void *wsi_connection, char const *file_name)
{
	renderer *new_renderer = new (malloc(sizeof(renderer))) renderer{};
	new_renderer->init(wsi_connection, file_name);
	return new_renderer;
}
...
void renderer::init(void *wsi_connection, char const *file_name)
{
