	...
	while (AInputQueue_getEvent(input_queue, &input_event) >= 0)
	{
		...
		if (0 == AInputQueue_preDispatchEvent(input_queue, input_event))
		{
			...
			case AINPUT_EVENT_TYPE_MOTION:
			{
				...
				float CurrentNormalized_X = Current_X / static_cast<float>(g_window_width);
				float CurrentNormalized_Y = Current_Y / static_cast<float>(g_window_height);
...
int main(int argc, char *argv[])
{
...
#ifndef NDEBUG
	{
		...
		char file_name[4096];
		ssize_t res_read_link = readlink("/proc/self/exe", file_name, sizeof(file_name) / sizeof(file_name[0]));
		...
		int res_set_env = setenv("VK_LAYER_PATH", file_name, 1);
		assert(0 == res_set_env);
	}
#endif
	...
	xcb_window_t window = 0;
	{
		...
		uint32_t value_list[5] = {screen->black_pixel, 0, XCB_BACKING_STORE_NOT_USEFUL, XCB_EVENT_MASK_STRUCTURE_NOTIFY, colormap};
...

	{
		...
		g_renderer = renderer_init(&brx_xcb_connection);
...
int wmain(int argc, wchar_t *argv[])
{
	...
	// Initialize
	{
		std::string file_name_utf8;
		{
			...
			{
				file_name_utf16 = argv[1];
			}

			bool res_convert_utf16_To_utf8_string = llvm::convertUTF16ToUTF8String(file_name_utf16, file_name_utf8);
		...
		g_renderer = renderer_init(NULL, file_name_utf8.c_str());
...
static LRESULT CALLBACK wnd_proc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MOUSEMOVE:
	{
		...
		float CurrentNormalized_X = static_cast<float>(Current_X) / g_window_width;
		float CurrentNormalized_Y = static_cast<float>(Current_Y) / g_window_height;
