					...
					for (size_t mesh_index = 0U; mesh_index < total_mesh_data.size(); ++mesh_index)
					{
						...
						for (size_t subset_index = 0U; subset_index < in_mesh_data.m_subsets.size(); ++subset_index)
						{
...

							{
								...
								if (in_subset_data.m_max_index <= static_cast<uint32_t>(UINT16_MAX))
								{
									gbuffer_pipeline_per_mesh_subset_update_set_geometry_material_storage_buffer_binding_destination.m_geometry_material_flags |= Geometry_Index_Type_UInt16;
								}
								if (!in_subset_data.m_normal_texture_image_uri.empty())
								{
									gbuffer_pipeline_per_mesh_subset_update_set_geometry_material_storage_buffer_binding_destination.m_geometry_material_flags |= Material_Texture_Enable_Normal;
								}
								if (!in_subset_data.m_emissive_texture_image_uri.empty())
								{
									gbuffer_pipeline_per_mesh_subset_update_set_geometry_material_storage_buffer_binding_destination.m_geometry_material_flags |= Material_Texture_Enable_Emissive;
								}
								if (!in_subset_data.m_base_color_texture_image_uri.empty())
								{
									gbuffer_pipeline_per_mesh_subset_update_set_geometry_material_storage_buffer_binding_destination.m_geometry_material_flags |= Material_Texture_Enable_Base_Color;
								}
								if (!in_subset_data.m_metallic_roughness_texture_image_uri.empty())
								{
									gbuffer_pipeline_per_mesh_subset_update_set_geometry_material_storage_buffer_binding_destination.m_geometry_material_flags |= Material_Texture_Enable_Metallic_Roughness;
								}
