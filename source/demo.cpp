...
#include <deferred_shading_fragment.inl>
    ...
    DirectX::XMFLOAT3 eye_position(0.19F, 1.70F, 7.44F);
    DirectX::XMFLOAT3 look_at_position(0.19F + 0.0F, 1.70F + 0.0F, 7.44F + -1.0F);
...
void Demo::draw(brx_graphics_command_buffer *command_buffer, float interval_time, uint32_t frame_throttling_index)
{
    ...
    // Skin Pass
    {
        ...
        if (mesh_skinned_subset_count > 0U)
        {
            ...
            mcrt_vector<BRX_COMPUTE_PASS_STORAGE_BUFFER_STORE_OPERATION> skinned_buffer_store_operations(skin_pipeline_buffers.size(), BRX_COMPUTE_PASS_STORAGE_BUFFER_STORE_OPERATION_FLUSH_FOR_READ_ONLY_STORAGE_BUFFER);
