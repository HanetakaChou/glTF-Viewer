			...
			for (size_t file_name_index = 0U; file_name_index < file_names.size(); ++file_name_index)
			{
				...
				if (import_scene_asset_gltf(total_mesh_data, frame_rate, file_name.c_str()))
				{
					for (size_t mesh_index = 0U; mesh_index < total_mesh_data.size(); ++mesh_index)
					{
						...
						this->m_scene_meshes.push_back({});
						Demo_Mesh &out_mesh = this->m_scene_meshes.back();
						...
						for (size_t subset_index = 0U; subset_index < in_mesh_data.m_subsets.size(); ++subset_index)
						{
							...
							out_mesh.m_subsets.push_back({});
							Demo_Mesh_Subset &out_subset = out_mesh.m_subsets.back();
...

							{
								...
								assert(in_subset_data.m_max_index < static_cast<uint32_t>(UINT16_MAX));
...

							{
								...
								gbuffer_pipeline_per_mesh_subset_update_set_material_storage_buffer_binding_destination.m_material_texture_enable_flags = 0U;
								if (!in_subset_data.m_normal_texture_image_uri.empty())
								{
									gbuffer_pipeline_per_mesh_subset_update_set_material_storage_buffer_binding_destination.m_material_texture_enable_flags |= Material_Texture_Enable_Normal;
								}
								if (!in_subset_data.m_emissive_texture_image_uri.empty())
								{
									gbuffer_pipeline_per_mesh_subset_update_set_material_storage_buffer_binding_destination.m_material_texture_enable_flags |= Material_Texture_Enable_Emissive;
								}
								if (!in_subset_data.m_base_color_texture_image_uri.empty())
								{
									gbuffer_pipeline_per_mesh_subset_update_set_material_storage_buffer_binding_destination.m_material_texture_enable_flags |= Material_Texture_Enable_Base_Color;
								}
								if (!in_subset_data.m_metallic_roughness_texture_image_uri.empty())
								{
									gbuffer_pipeline_per_mesh_subset_update_set_material_storage_buffer_binding_destination.m_material_texture_enable_flags |= Material_Texture_Enable_Metallic_Roughness;
								}
								gbuffer_pipeline_per_mesh_subset_update_set_material_storage_buffer_binding_destination.m_normal_texture_scale = in_subset_data.m_normal_texture_scale;
								gbuffer_pipeline_per_mesh_subset_update_set_material_storage_buffer_binding_destination.m_emissive_factor_x = in_subset_data.m_emissive_factor.x;
								gbuffer_pipeline_per_mesh_subset_update_set_material_storage_buffer_binding_destination.m_emissive_factor_y = in_subset_data.m_emissive_factor.y;
								gbuffer_pipeline_per_mesh_subset_update_set_material_storage_buffer_binding_destination.m_emissive_factor_z = in_subset_data.m_emissive_factor.z;
								gbuffer_pipeline_per_mesh_subset_update_set_material_storage_buffer_binding_destination.m_base_color_factor_x = in_subset_data.m_base_color_factor.x;
								gbuffer_pipeline_per_mesh_subset_update_set_material_storage_buffer_binding_destination.m_base_color_factor_y = in_subset_data.m_base_color_factor.y;
								gbuffer_pipeline_per_mesh_subset_update_set_material_storage_buffer_binding_destination.m_base_color_factor_z = in_subset_data.m_base_color_factor.z;
								gbuffer_pipeline_per_mesh_subset_update_set_material_storage_buffer_binding_destination.m_metallic_factor = in_subset_data.m_metallic_factor;
								gbuffer_pipeline_per_mesh_subset_update_set_material_storage_buffer_binding_destination.m_roughness_factor = in_subset_data.m_roughness_factor;

								uint32_t const material_buffer_size = sizeof(gbuffer_pipeline_per_mesh_subset_update_set_material_storage_buffer_binding);
								...
								std::memcpy(material_staging_upload_buffer->get_host_memory_range_base(), &gbuffer_pipeline_per_mesh_subset_update_set_material_storage_buffer_binding_destination, material_buffer_size);
...

							{
								...
								for (uint32_t mesh_subset_meterial_texture_index = 0U; mesh_subset_meterial_texture_index < DEMO_MESH_SUBSET_METERIAL_TEXTURE_COUNT; ++mesh_subset_meterial_texture_index)
								{
									...
									if (!meterial_texture_image_uri.empty())
									{
										...
										brx_staging_upload_buffer *image_staging_upload_buffer;
										{
											std::string image_asset_file_name;
											{
												...
												image_asset_file_name += "dds";
											}

											import_asset_input_stream_file import_image_asset_file_input_stream;
											import_image_asset_file_input_stream.init(image_asset_file_name.c_str());

											size_t image_asset_data_offset;
											bool const res_load_dds_image_asset_header = import_image_asset_header_from_input_stream_dds(&import_image_asset_file_input_stream, &image_asset_header, &image_asset_data_offset);
											...
											uint32_t total_bytes = brx_sampled_asset_image_import_calculate_subresource_memcpy_dests(image_asset_header.format, image_asset_header.width, image_asset_header.height, 1U, image_asset_header.mip_levels, 1U, 0U, staging_upload_buffer_offset_alignment, staging_upload_buffer_row_pitch_alignment, subresource_count, &subresource_memcpy_dests[0]);
											...
											bool const load_image_asset_data = import_image_asset_data_from_input_stream_dds(&import_image_asset_file_input_stream, &image_asset_header, image_asset_data_offset, image_staging_upload_buffer->get_host_memory_range_base(), subresource_count, &subresource_memcpy_dests[0]);
											assert(load_image_asset_data);
						...
						for (size_t instance_index = 0U; instance_index < total_mesh_data[mesh_index].m_instances.size(); ++instance_index)
						{
							...
							out_mesh.m_instances.push_back({});
							Demo_Mesh_Instance &out_mesh_instance = out_mesh.m_instances.back();
			...
			for (size_t subset_index = 0U; subset_index < scene_mesh.m_subsets.size(); ++subset_index)
			{
				...
				for (size_t instance_index = 0U; instance_index < scene_mesh.m_instances.size(); ++instance_index)
				{
					...
					brx_descriptor_set *const descritor_sets[] = {
						...
						scene_mesh_subset.m_gbuffer_pipeline_per_mesh_subset_update_descriptor_set,
