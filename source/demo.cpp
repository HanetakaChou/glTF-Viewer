...
#include "../thirdparty/ImportAsset/thirdparty/McRT-Malloc/include/mcrt_unordered_map.h"
                        ...
                        for (size_t mesh_index = 0U; mesh_index < total_mesh_data.size(); ++mesh_index)
                        {
                            ...
                            for (size_t subset_index = 0U; subset_index < in_mesh_data.m_subsets.size(); ++subset_index)
                            {
                                ...
                                // Buffer
                                {
                                    ...
                                    gbuffer_pipeline_per_mesh_subset_update_set_information_storage_buffer_binding gbuffer_pipeline_per_mesh_subset_update_set_information_storage_buffer_binding_source;
                                    {
                                        gbuffer_pipeline_per_mesh_subset_update_set_information_storage_buffer_binding_source.m_geometry_material_flags = 0U;
                                        if (in_subset_data.m_max_index <= static_cast<uint32_t>(UINT16_MAX))
                                        {
                                            gbuffer_pipeline_per_mesh_subset_update_set_information_storage_buffer_binding_source.m_geometry_material_flags |= Geometry_Flag_Index_Type_UInt16;
                                        }
                                        if (!in_subset_data.m_normal_texture_image_uri.empty())
                                        {
                                            gbuffer_pipeline_per_mesh_subset_update_set_information_storage_buffer_binding_source.m_geometry_material_flags |= Material_Flag_Enable_Normal_Texture;
                                        }
                                        if (!in_subset_data.m_emissive_texture_image_uri.empty())
                                        {
                                            gbuffer_pipeline_per_mesh_subset_update_set_information_storage_buffer_binding_source.m_geometry_material_flags |= Material_Flag_Enable_Emissive_Texture;
                                        }
                                        if (!in_subset_data.m_base_color_texture_image_uri.empty())
                                        {
                                            gbuffer_pipeline_per_mesh_subset_update_set_information_storage_buffer_binding_source.m_geometry_material_flags |= Material_Flag_Enable_Base_Colorl_Texture;
                                        }
                                        if (!in_subset_data.m_metallic_roughness_texture_image_uri.empty())
                                        {
                                            gbuffer_pipeline_per_mesh_subset_update_set_information_storage_buffer_binding_source.m_geometry_material_flags |= Material_Flag_Enable_Metallic_Roughness_Texture;
                                        }
                                        gbuffer_pipeline_per_mesh_subset_update_set_information_storage_buffer_binding_source.m_normal_texture_scale = in_subset_data.m_normal_texture_scale;
                                        gbuffer_pipeline_per_mesh_subset_update_set_information_storage_buffer_binding_source.m_emissive_factor_x = in_subset_data.m_emissive_factor.x;
                                        gbuffer_pipeline_per_mesh_subset_update_set_information_storage_buffer_binding_source.m_emissive_factor_y = in_subset_data.m_emissive_factor.y;
                                        gbuffer_pipeline_per_mesh_subset_update_set_information_storage_buffer_binding_source.m_emissive_factor_z = in_subset_data.m_emissive_factor.z;
                                        gbuffer_pipeline_per_mesh_subset_update_set_information_storage_buffer_binding_source.m_base_color_factor_x = in_subset_data.m_base_color_factor.x;
                                        gbuffer_pipeline_per_mesh_subset_update_set_information_storage_buffer_binding_source.m_base_color_factor_y = in_subset_data.m_base_color_factor.y;
                                        gbuffer_pipeline_per_mesh_subset_update_set_information_storage_buffer_binding_source.m_base_color_factor_z = in_subset_data.m_base_color_factor.z;
                                        gbuffer_pipeline_per_mesh_subset_update_set_information_storage_buffer_binding_source.m_metallic_factor = in_subset_data.m_metallic_factor;
                                        gbuffer_pipeline_per_mesh_subset_update_set_information_storage_buffer_binding_source.m_roughness_factor = in_subset_data.m_roughness_factor;
                                    }
                                    ...
                                    size_t const source_asset_buffer_sizes[DEMO_MESH_SUBSET_ASSET_BUFFER_COUNT] = {
                                        ...
                                        (in_subset_data.m_max_index <= static_cast<uint32_t>(UINT16_MAX)) ? (sizeof(uint16_t) * out_subset.m_index_count) : (sizeof(uint32_t) * out_subset.m_index_count),
                                        sizeof(gbuffer_pipeline_per_mesh_subset_update_set_information_storage_buffer_binding)};
                                    ...
                                    void const *const source_asset_buffers[DEMO_MESH_SUBSET_ASSET_BUFFER_COUNT] = {
                                        ...
                                        (in_subset_data.m_max_index <= static_cast<uint32_t>(UINT16_MAX)) ? static_cast<void const *>(uint16_indices.data()) : static_cast<void const *>(in_subset_data.m_indices.data()),
                                        &gbuffer_pipeline_per_mesh_subset_update_set_information_storage_buffer_binding_source};
...
void Demo::on_swap_chain_attach(brx_device *device, uint32_t swap_chain_image_width, uint32_t swap_chain_image_height)
{
    ...
    // Camera
    g_camera.SetProjParams(DirectX::XM_PI * (1.0F / 3.0F), static_cast<float>(this->m_intermediate_width) / static_cast<float>(this->m_intermediate_height), 0.1F, 1000.0F);
}
...
void Demo::draw(brx_graphics_command_buffer *command_buffer, float interval_time, uint32_t frame_throttling_index)
{
    ...
    // Skin Pass
    {
        mcrt_vector<brx_storage_buffer const *> skinned_buffers;
        ...
        if (!skinned_buffers.empty())
        {
            ...
            mcrt_vector<BRX_COMPUTE_PASS_STORAGE_BUFFER_LOAD_OPERATION> skinned_buffer_load_operations(skinned_buffers.size(), BRX_COMPUTE_PASS_STORAGE_BUFFER_LOAD_OPERATION_DONT_CARE);
            command_buffer->compute_pass_load(static_cast<uint32_t>(skinned_buffers.size()), skinned_buffers.data(), skinned_buffer_load_operations.data(), 0U, NULL, NULL);
                ...
                if (scene_mesh.m_skinned)
                {
                    for (size_t instance_index = 0U; instance_index < scene_mesh.m_instances.size(); ++instance_index)
                    {
                        ...
                        for (size_t subset_index = 0U; subset_index < scene_mesh.m_subsets.size(); ++subset_index)
                        {
                            ...
                            uint32_t const group_count_x = (scene_mesh_subset.m_vertex_count <= MAX_SKIN_COMPUTE_DISPATCH_THREAD_GROUPS_PER_DIMENSION) ? scene_mesh_subset.m_vertex_count : MAX_SKIN_COMPUTE_DISPATCH_THREAD_GROUPS_PER_DIMENSION;
                            uint32_t const group_count_y = (scene_mesh_subset.m_vertex_count + MAX_SKIN_COMPUTE_DISPATCH_THREAD_GROUPS_PER_DIMENSION - 1U) / MAX_SKIN_COMPUTE_DISPATCH_THREAD_GROUPS_PER_DIMENSION;
            ...
            mcrt_vector<BRX_COMPUTE_PASS_STORAGE_BUFFER_STORE_OPERATION> skinned_buffer_store_operations(skinned_buffers.size(), BRX_COMPUTE_PASS_STORAGE_BUFFER_STORE_OPERATION_FLUSH_FOR_READ_ONLY_STORAGE_BUFFER);
            command_buffer->compute_pass_store(static_cast<uint32_t>(skinned_buffers.size()), skinned_buffers.data(), skinned_buffer_store_operations.data(), 0U, NULL, NULL);
