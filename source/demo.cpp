...
#include <deferred_shading_fragment.inl>
    ...
    // Asset & Place Holder Texture
    {
...

        {
            ...
            // Asset
            {
                std::unordered_map<std::string, brx_sampled_asset_image *> mapped_textures;
...

                                {
                                    ...
                                    for (uint32_t mesh_subset_meterial_texture_index = 0U; mesh_subset_meterial_texture_index < DEMO_MESH_SUBSET_METERIAL_TEXTURE_COUNT; ++mesh_subset_meterial_texture_index)
                                    {
                                        ...
                                        if (!meterial_texture_image_uri.empty())
                                        {
                                            ...
                                            std::unordered_map<std::string, brx_sampled_asset_image *>::const_iterator found;
                ...
                for (std::unordered_map<std::string, brx_sampled_asset_image *>::const_iterator material_texture_iterator = mapped_textures.begin(); material_texture_iterator != mapped_textures.end(); ++material_texture_iterator)
                {
            ...
            // acquire
            {
                ...
                for (std::vector<brx_sampled_asset_image *>::const_iterator material_texture_iterator = this->m_scene_textures.begin(); material_texture_iterator != this->m_scene_textures.end(); ++material_texture_iterator)
                {
...
void Demo::draw(brx_graphics_command_buffer *command_buffer, float interval_time, uint32_t frame_throttling_index)
{
    // Update Uniform Buffer
    {
        // Skin Pipeline - Per Mesh Instance Update
        {
            ...
            // 60 FPS
            size_t animetion_frame_index = static_cast<size_t>(60.0 * this->m_animation_time);
    ...
    // Skin Pass
    {
        std::vector<brx_storage_buffer const *> skinned_buffers;
        ...
        if (!skinned_buffers.empty())
        {
            ...
            std::vector<BRX_COMPUTE_PASS_STORAGE_BUFFER_LOAD_OPERATION> skinned_buffer_load_operations(skinned_buffers.size(), BRX_COMPUTE_PASS_STORAGE_BUFFER_LOAD_OPERATION_DONT_CARE);
            ...
            std::vector<BRX_COMPUTE_PASS_STORAGE_BUFFER_STORE_OPERATION> skinned_buffer_store_operations(skinned_buffers.size(), BRX_COMPUTE_PASS_STORAGE_BUFFER_STORE_OPERATION_FLUSH_FOR_READ_ONLY_STORAGE_BUFFER);
