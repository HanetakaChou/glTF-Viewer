...
{
    brx_uint index_buffer_offset = (0u == ((g_index_buffer_stride * brx_uint(brx_vertex_id)) % 4u)) ? (g_index_buffer_stride * brx_uint(brx_vertex_id)) : ((g_index_buffer_stride * brx_uint(brx_vertex_id)) - 2u);
    ...
    brx_uint vertex_index = (0u == ((g_index_buffer_stride * brx_uint(brx_vertex_id)) % 4u)) ? unpacked_vector_index_buffer.x : unpacked_vector_index_buffer.y;
    
    brx_uint vertex_position_buffer_offset = g_vertex_position_buffer_stride * vertex_index;
    brx_float3 vertex_position_model_space = brx_uint_as_float(brx_byte_address_buffer_load3(g_geometry_material_buffers[0], vertex_position_buffer_offset));
