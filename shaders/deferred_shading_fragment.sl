...
brx_pixel_shader_parameter_end(main)
{
	...
	brx_float3 f0 = brx_clamp(brx_lerp(brx_float3(dielectric_specular, dielectric_specular, dielectric_specular), base_color, metallic), brx_float3(0.0, 0.0, 0.0), brx_float3(1.0, 1.0, 1.0));
	brx_float3 albedo = brx_clamp(base_color - f0, brx_float3(0.0, 0.0, 0.0), brx_float3(1.0, 1.0, 1.0));
	...
	const brx_int incident_light_count = 3;

	const brx_float3 incident_illuminances[incident_light_count] = brx_array_constructor_begin(brx_float3, 3) 
		brx_float3(1.0, 1.0, 1.0) brx_array_constructor_split
		brx_float3(1.0, 1.0, 1.0) brx_array_constructor_split
		brx_float3(1.0, 1.0, 1.0)
        brx_array_constructor_end;

	const brx_float3 Ls[incident_light_count] = brx_array_constructor_begin(brx_float3, 3) 
		brx_float3( 0.0      , 0.8660254, -0.5 ) brx_array_constructor_split
		brx_float3( 0.4330127, 0.8660254,  0.25) brx_array_constructor_split
		brx_float3(-0.4330127, 0.8660254,  0.25)
        brx_array_constructor_end;

	brx_unroll
	for(brx_int incident_light_index = 0; incident_light_index < 3; ++incident_light_index)
	{
