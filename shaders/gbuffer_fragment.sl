...
brx_pixel_shader_parameter_end(main)
{
    ...
    packed_material_buffer_vectors[0] = brx_byte_address_buffer_load4(g_asset_buffers[3], 0);
    packed_material_buffer_vectors[1] = brx_byte_address_buffer_load4(g_asset_buffers[3], 4 * 4);
    packed_material_buffer_vectors[2].xy = brx_byte_address_buffer_load2(g_asset_buffers[3], 4 * 4 + 4 * 4);
    ...
    if(0u != (geometry_material_flags & Material_Flag_Enable_Normal_Texture))
    {
        ...
        brx_float3 shading_normal_tangent_space = brx_normalize((brx_sample_2d(g_asset_textures[0], g_sampler[0], texcoord).xyz * 2.0 - brx_float3(1.0, 1.0, 1.0)) * brx_float3(normal_texture_scale, normal_texture_scale, 1.0));
    ...
    {
        emissive *= brx_sample_2d(g_asset_textures[1], g_sampler[0], texcoord).xyz;
    }
    ...
    {
        base_color *= brx_sample_2d(g_asset_textures[2], g_sampler[0], texcoord).xyz;
    }
    ...
    {
        brx_float2 metallic_roughness = brx_sample_2d(g_asset_textures[3], g_sampler[0], texcoord).bg;
